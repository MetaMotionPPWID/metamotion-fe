...
const GyroscopeGraph = ({
  ...
  const chartData = {
    ...
    datasets: [
      ...(visibleAxes.x
        ? [{ data: dataPoints.map(p => p.x), color: () => "rgba(255,0,0,0.8)", strokeWidth: 2 }]
        : []),
      ...(visibleAxes.y
        ? [{ data: dataPoints.map(p => p.y), color: () => "rgba(0,255,0,0.8)", strokeWidth: 2 }]
        : []),
      ...(visibleAxes.z
        ? [{ data: dataPoints.map(p => p.z), color: () => "rgba(0,0,255,0.8)", strokeWidth: 2 }]
        : []),
    ],
    legend: [
      ...(visibleAxes.x ? ["X"] : []),
...
const styles = StyleSheet.create({
  container: { padding: 16, backgroundColor: "white", borderRadius: 8, elevation: 2, shadowColor: "#000", shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.2, shadowRadius: 1 },
  title: { fontSize: 18, fontWeight: "bold", marginBottom: 12, textAlign: "center" },
  chart: { marginVertical: 8, borderRadius: 16 },
  placeholderChart: { height: 220, backgroundColor: "#f5f5f5", borderRadius: 16, justifyContent: "center", alignItems: "center" },
  placeholderText: { color: "#666", fontSize: 16 },
  axisControls: { flexDirection: "row", justifyContent: "center", marginTop: 12 },
  axisButton: { paddingVertical: 8, paddingHorizontal: 20, borderRadius: 20, marginHorizontal: 8, backgroundColor: "#eee" },
  ...
  currentValues: { marginTop: 16, backgroundColor: "#f5f5f5", padding: 12, borderRadius: 8 },
